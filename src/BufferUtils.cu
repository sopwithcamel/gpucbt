...
namespace gpucbt {
    ...
    void Buffer::GPUSort(uint32_t num) {
        ...
        thrust::device_vector<Message> d_agg_msg(num);
        // gather messages according to permutations
        thrust::gather(d_perm.begin(), d_perm.end(), d_msg.begin(),
                d_agg_msg.begin());
        ...
        try {
            new_end = thrust::reduce_by_key(d_hash.begin(), d_hash.end(),
                    d_agg_msg.begin(), d_hash.begin(), d_agg_msg.begin(),
                    thrust::equal_to<int>(),
        ...
        thrust::copy(d_agg_msg.begin(), new_end.second, messages_);
